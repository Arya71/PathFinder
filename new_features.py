...
def generate_random_obstacles(grid, start_node, end_node, density=0.3):
    ...
    Preserves start/end nodes if set.
    """
    for row in grid:
        for node in row:
            node.reset()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    started = True
                    result = algorithms[selected_algorithm_name]["func"](
                        lambda: draw_main(win, grid, selected_algorithm_name, stats, algorithms, diagonal_allowed), 
                        grid, start, end
                    )
                ...
                # Algorithm Selection moved to 6,7,8,9,0 to avoid conflict with density keys
